...
def calc_3d_metric(rec_meshfile, gt_meshfile, align=True):
    ...
    return {'accuracy': accuracy_rec,
            'completion': completion_rec,
    'completion ratio': completion_ratio_rec}
...
def calc_2d_metric(rec_meshfile, gt_meshfile, align=True, n_imgs=1000):
    ...
    for i in range(n_imgs):
        while True:
            ...
            target = [tx, ty, tz] # will be normalized, so sample from range [0.0,1.0]
            target = np.array(target)-np.array(origin)
            c2w = viewmatrix(target, up, origin)
            tmp = np.eye(4)
            tmp[:3, :] = c2w # sample translations
            c2w = tmp
            seen = check_proj(pc_unseen, W, H, fx, fy, cx, cy, c2w) # if unseen points are projected into current view (c2w)
            if (~seen):
                break
        ...
        if (ours_depth > 0).sum() > 0:
            errors += [np.abs(gt_depth[ours_depth > 0] - ours_depth[ours_depth > 0]).mean()]
        else:
            continue
...
if __name__ == '__main__':
    ...
    try:
        ...
        if args.metric_2d:
            result_2d = calc_2d_metric(args.rec_mesh, args.gt_mesh, align=not args.no_align, n_imgs=1000)
